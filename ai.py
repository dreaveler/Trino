...
class AI:
    ...
    def ai_choose_to_use_skill(self, player, skill_name: str) -> bool:
        """AI选择是否使用技能"""
        # AI策略：根据技能类型和当前情况决定是否使用
        if skill_name == '奸雄':
            # 如果有+2/+4牌可以获取，就使用
            return len(player.game.draw_chain_cards) > 0
        elif skill_name == '武圣':
            # 如果有红色牌且当前需要+2效果，就使用
            red_cards = [c for c in player.uno_list if c.color == 'red']
            return len(red_cards) > 0
        elif skill_name == '反间':
            # 如果有非黑色牌且其他玩家手牌较多，就使用
            non_black_cards = [c for c in player.uno_list if c.color != 'black']
            other_players = [p for p in player.game.player_list if p != player]
            max_hand = max(len(p.uno_list) for p in other_players) if other_players else 0
            return len(non_black_cards) > 0 and max_hand > 3
        elif skill_name == '缔盟':
            # 如果手牌数<=6且其他玩家手牌数差异较大，就使用
            other_players = [p for p in player.game.player_list if p != player]
            if len(other_players) >= 2:
                hand_counts = [len(p.uno_list) for p in other_players]
                return len(player.uno_list) <= 6 and max(hand_counts) - min(hand_counts) >= 2
            return False
        else:
            # 默认不使用技能
                    return False
